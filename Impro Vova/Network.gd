...
onready var lobby = "res://Lobby.tscn"
...
func join_server(to_ip):
	to_ip = "192.168.0.101:9080" #192.168.0.111:9080
...
func _connected(proto = ""):
	...
	send_data("Vova connected")
	
	get_tree().change_scene(main)


func _on_data():
	data = _client.get_peer(1).get_packet().get_string_from_utf8()
	if data == "!timer80":
		emit_signal("timer80")
	elif data == "!timer45":
		emit_signal("timer45")
	elif data == "!timer420":
		emit_signal("timer420")
	else:
		get_node("/root/Main/LabelInfo").text = data
